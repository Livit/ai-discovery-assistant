...
// ─────────────────────────────────────────────────────────────
// REFINEMENT — multi-select chips + Apply (frictionless flow)
//   Sits UNDER the recommendations. Selections only take effect on Apply.
// ─────────────────────────────────────────────────────────────
function RefineChips({ refine, onRefine }) {
  const [sel, setSel] = useDS([]);
  function toggle(opt) {
    setSel((s) => s.includes(opt) ? s.filter((x) => x !== opt) : [...s, opt]);
  }
  function apply() {
    if (!sel.length || !onRefine) return;
    const prefix = refine.factor ? `${refine.factor}: ` : "";
    onRefine(prefix + sel.join(", "));
  }
  return (
    <div className="ai-refine">
      <div className="ai-refine-head">
        <span className="ai-refine-spark"><HeroSparkles size={18} /></span>
        <span className="ai-refine-prompt">{refine.prompt}</span>
      </div>
      <div className="ai-refine-row">
        <div className="ai-refine-chips">
          {refine.options.map((opt) => {
            const on = sel.includes(opt);
            return (
              <button
                key={opt}
                className={`ai-refine-chip ${on ? "is-on" : ""}`}
                aria-pressed={on}
                onClick={() => toggle(opt)}>
                {on && <IDr name="check" />}{opt}
              </button>);
          })}
        </div>
        <button
          className="ai-refine-apply"
          onClick={apply}
          disabled={!sel.length}>
          Apply{sel.length ? ` (${sel.length})` : ""}
        </button>
      </div>
    </div>);
}
...
function AIRecs({ intro, strong, partial, refine, onRefine, addedSims, setAddedSims, onToggle }) {
  ...
  return (
    <div className="ai-results">
      ...
      {refine && refine.options && refine.options.length > 0 && !empty &&
      <RefineChips refine={refine} onRefine={onRefine} />
      }
    </div>);
  ...
  async function aiSend(text) {
    ...
    try {
      ...
      if (res.type === "recommendations") {
        ...
        setAiThread((prev) => [...prev, { kind: "recs", intro: res.intro, strong, partial, refine: res.refine }]);
