  ...
  return (
    <aside className="sr-filter-rail">
      <FilterGroup title="Type">
        {["Simulations", "Units", "Practice activities", "Courses"].map((v) =>
        <FilterCheck key={v} label={v} checked={has("type", v)} onChange={() => toggle("type", v)} />
        )}
      </FilterGroup>
...
// ─────────────────────────────────────────────────────────────
function SearchResultsView({ query, results, interp, onRefineInterp, onBack }) {
  ...
  return (
    <div className="sr-wrap" data-screen-label="03 Search results">
      ...
      {interp && interp.length > 0 &&
      <div className="sr-ai-banner" style={{ textAlign: "right" }}>
          <AIBanner query={query} terms={interp} onRefine={onRefineInterp} />
        </div>
      }
      ...
      <div className="sr-main">
        ...
        <div className="sr-results-col">
          {results.map((r, i) => {
            const kind = r.confidence || (r.relation ? "related" : null);
            const note = kind === "related" ? r.relation : r.match;
            const variant = { strong: "success", partial: "warning", related: "info" }[kind];
            return (
              <div key={r.id} className="sr-result-group">
                {variant && <ResultToast variant={variant} text={note} />}
  ...
  return (
    <div className="st-stage st-stage-results">
      ...
      <div className="st-ai-warning">
        <div className="st-ai-warning-icon"><ICat name="triangle-alert" /></div>
...
function CatalogPage() {
  ...
  const [mode, setMode] = useCatS("browse"); // browse | results-nl | results-zero
  ...
  useCatE(() => {
    window.AICatalog = {
      ...
      nlResults: () => onSubmit("a simulation that helps students understand osmosis before a wet lab"),
      zeroResults: () => onSubmit("human factors engineering lab simulation"),
  ...
  function onSubmit(query) {
    ...
    if (ql.includes("osmosis") || ql.includes("wet lab") || ql.includes("membrane transport") || ql.includes("diffusion")) {
      ...
      // fallback — return to browse and let the search box filter
      setMode("browse");
    }
  ...
  const hasResults = mode === "results-nl" || mode === "results-zero";
